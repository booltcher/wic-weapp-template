...
Page({
  data: {
    ...
    formRules: [
      ...
      { prop: 'mobile', validator: 'regexp:mobile', msg: '请输入正确的联系电话' },
      {
        prop: 'addressName', validator: (value) => {
  ...
  async submit() {
    ...
    Validator(this.data.info, this.data.formRules).then(async res => {
      LoadingToast()
      try {
        let res = this.data.info.id ? await services.modifyAddress(formData) : await services.createAddress(formData)
        if (res.code === '200') {
          SuccessToast('保存成功')
          setTimeout(function () {
            wx.navigateBack()
