...
const FormValidator = (data, rules, options = {}) => {
  return new Promise((resolve, reject) => {
    ...
    for (let i = 0; i < rules.length; i++) {
      ...
      if (data.hasOwnProperty(prop)) {
        let result = _Validate_(
          { prop, value, validator, msg },
          regExprssions,
          errorMode
        );
...
/**
 * 验证主体方法，支持 required, 正则, 自定义校验函数
 */
const _Validate_ = (regular, regExprssions, errorMode) => {
  ...
  if (regExprssions[regexp]) {
    return regExprssions[regexp].test(value)
      ? true
      : errorHandle(prop, msg, errorMode);
  }
};
